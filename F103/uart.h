...
#define _UART_rx_count(num)	(uart_buf_size(&UART_buf(num,rx)))
#define _UART_tx_count(num)  (UART_MASK - uart_buf_size(&UART_buf(num,rx)))
...
#define UART_init(num, baud) do{\
  ...
  if(num == 1){\
    ...
    if(num == 2)RCC->APB1ENR |= RCC_APB1ENR_USART2EN;\
    if(num == 3)RCC->APB1ENR |= RCC_APB1ENR_USART3EN;\
